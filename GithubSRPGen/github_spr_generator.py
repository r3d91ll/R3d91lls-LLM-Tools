...
class DirectoryManager:
    ...
    def create_directories(self):
        """
        Creates and mirrors the structure for the SPREnabled and working directories.
        """
    ...
    def _mirror_directory_structure(self, source, destination):
        for item in source.iterdir():
            ...
            elif item.is_file() and item.suffix not in ['.md', '.ipynb']:
                if destination == self.spr_directory:
...
class ContentProcessor:
    def __init__(self, clone_directory, spr_directory):
        """
        Initializes the ContentProcessor with directory paths.
        :param clone_directory: The directory where the repository is cloned.
    ...
    def process_content(self):
        ...
        for item in self.clone_directory.rglob('*'):
            if item.is_file():
                if item.suffix == '.md':
                    self.markdown_processor.process_markdown_files(item)
                elif item.suffix == '.ipynb':
                    self.jupyter_processor.process_jupyter_notebooks(item)
                else:
...
class JupyterProcessor:
    def __init__(self, clone_directory, working_directory):
        """
        Initializes the JupyterProcessor with directory paths.
        :param clone_directory: The directory where the repository is cloned.
        :param working_directory: The directory for storing processed content.
        """
        self.clone_directory = Path(clone_directory)
        self.working_directory = Path(working_directory)
    ...
    def process_jupyter_notebooks(self):
        """
        Processes all Jupyter Notebook files in the cloned directory.
        """
    ...
    def _process_single_notebook(self, file_path):
        """
        Converts and processes a single Jupyter Notebook to Markdown format.
        :param file_path: Path to the Jupyter Notebook file.
        """
        with open(file_path, 'r', encoding='utf-8') as f:
            nb = nbformat.read(f, as_version=4)

        markdown_content = self._convert_notebook_to_markdown(nb)
        
        # Optionally, you can process the converted Markdown further, e.g., delinting
        processed_markdown = self._process_converted_markdown(markdown_content)

        # Save the processed content to the working_directory
...
def main():
    ...
    content_processor = ContentProcessor(git_manager.clone_directory, directory_manager.working_directory)
